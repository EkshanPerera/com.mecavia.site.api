...
@Service
@Transactional
public class PCSMappingServiceImpl  implements com.mecavia.site.service.PCSMappingService{
	@Autowired
	private ProductRepoV1 productrepo;
	...
	@Autowired
	private ExtendedProductServiceImpl productService; 
	...
	public PCSMappingDto getPCSMById(int id) {
		if (pcsmrepo.existsById(id)) {
			...
			try {
				...
				pcsm.setColourDto(colourServiceImpl.getColour(String.valueOf(colourid)));
				pcsm.setSizeDto(sizeServiceImpl.getSize(String.valueOf(sizeid)));
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}
			return pcsm;
