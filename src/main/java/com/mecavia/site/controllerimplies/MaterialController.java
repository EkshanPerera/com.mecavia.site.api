...
import com.mecavia.site.serviceimplies.MaterialService;
...
public class MaterialController implements com.mecavia.site.controller.MaterialController {
	...
	public ResponseEntity<ResponseDto> updateMaterial(MaterialDto materialDto) {
		try {
			String res =  materialService.updateMaterial(materialDto);
			if(res.equals("00")) {
				responseDto.setCode(VarList.RSP_SUCCESS);
				responseDto.setMessage("success");
				responseDto.setContent(materialDto);
	...
	public ResponseEntity<ResponseDto> activeinactiveMaterial(ActiveInactiveEntityDto activeInactiveEntityDto) {
		try {
			String res =  materialService.activeInactiveMaterial(activeInactiveEntityDto);
			if(res.equals("00")) {
				responseDto.setCode(VarList.RSP_SUCCESS);
				responseDto.setMessage("success");
	...
	public ResponseEntity<ResponseDto> getMaterial(String materialid) {
		try {
			MaterialDto material = materialService.getMaterial(Integer.parseInt(materialid));
			if(material!=null) {
				responseDto.setCode(VarList.RSP_SUCCESS);
				responseDto.setMessage("success");
	...
	public ResponseEntity<ResponseDto> getMaterials() {
		...
		try {
			List<MaterialDto> materialList = materialService.getMaterials();
