...
public class UserGroupController implements com.mecavia.site.controller.UserGroupController {
	...
	public ResponseEntity<ResponseDto> saveUserGroup(UserGroupDto userGroupDto) {
		try {
			String res = userGroupService.saveUserGroup(userGroupDto);
			if(res.equals("00")) {
	...
	public ResponseEntity<ResponseDto> updateUserGroup(UserGroupDto userGroupDto) {
		try {
			String res = userGroupService.updateUserGroup(userGroupDto);
			if(res.equals("00")) {
				responseDto.setCode(VarList.RSP_SUCCESS);
				responseDto.setMessage("success");
				responseDto.setContent(userGroupDto);
	...
	public ResponseEntity<ResponseDto> activeInactiveUserGroup(ActiveInactiveEntityDto activeInactiveEntityDto) {
		try {
			String res = userGroupService.activeinactiveUserGroup(activeInactiveEntityDto);
			if(res.equals("00")) {
				responseDto.setCode(VarList.RSP_SUCCESS);
				responseDto.setMessage("success");
	...
	public ResponseEntity<ResponseDto> getUserGroups() {
		try {
			List<UserGroupDto> res = userGroupService.getUserGroups();
			if(res.isEmpty()) {
				responseDto.setCode(VarList.RSP_NO_DATA_FOUND);
	...
	public ResponseEntity<ResponseDto> getUserGroup(String usergroupid) {
		try {
			UserGroupDto res = userGroupService.getuserGroup(Integer.parseInt(usergroupid));
			if(res==null) {
				responseDto.setCode(VarList.RSP_NO_DATA_FOUND);
				responseDto.setMessage("No data found");
				responseDto.setContent(null);
