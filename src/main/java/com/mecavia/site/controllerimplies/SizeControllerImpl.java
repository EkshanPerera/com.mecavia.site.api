...
import com.mecavia.site.controller.SizeController;
import com.mecavia.site.dto.ActiveInactiveEntityDto;
import com.mecavia.site.dto.ResponseDto;
import com.mecavia.site.dto.SizeDto;
import com.mecavia.site.serviceimplies.SizeServiceImpl;
...
public class SizeControllerImpl implements SizeController {
	...
	public ResponseEntity<ResponseDto> getSizes() {
		try {
			List<SizeDto> size  = sizeServiceImpl.getSizes();
			if(size.isEmpty()) {
	...
	public ResponseEntity<ResponseDto> saveSize(SizeDto sizedto) {
		try {
			String res = sizeServiceImpl.saveSize(sizedto);
			if(res.equals("00")) {
				responseDto.setCode(VarList.RSP_SUCCESS);
				responseDto.setMessage("success");
	...
	public ResponseEntity<ResponseDto> updateSize(SizeDto sizedto) {
		try {
			String res = sizeServiceImpl.updateSize(sizedto);
			if(res.equals("00")) {
				responseDto.setCode(VarList.RSP_SUCCESS);
				responseDto.setMessage("success");
				responseDto.setContent(sizedto);
	...
	public ResponseEntity<ResponseDto> getSize(String sizeid) {
		try {
			SizeDto size  = sizeServiceImpl.getSize(sizeid);
			if(size == null) {
				responseDto.setCode(VarList.RSP_NO_DATA_FOUND);
				responseDto.setMessage("no daa found");
				responseDto.setContent(null);
	...
	public ResponseEntity<ResponseDto> activeinactiveSize(ActiveInactiveEntityDto aiedto) {
		try {
			String res = sizeServiceImpl.activeinactiveSize(aiedto);
			if(res.equals("00")) {
				responseDto.setCode(VarList.RSP_SUCCESS);
				responseDto.setMessage("success");
